...
class TaskEncoder(
    ...
    def encode_vqa(self, sample: VQASample):
        ...
        # Use the underlying HuggingFace tokenizer's .encode() to get integer IDs.
        # The chat template already embeds all special tokens in the string, so
        # add_special_tokens=False avoids double-adding BOS/EOS.
        input_ids = self.tokenizer.tokenizer.encode(text, add_special_tokens=False)
        user_input_ids = self.tokenizer.tokenizer.encode(user_inputs, add_special_tokens=False)
    ...
    def batch(self, samples: list[ImageTaskSample]) -> VQATaskBatch:
        ...
        ####################################################
        # NOTE(lizhiyu): use the transformers processor
        # s.imgs may be a single ndarray (encode_sample path) or a list of ndarrays
        # (encode_vqa path via _flatten_visual_inputs); handle both to avoid mismatch
        # with image_thw_grids.
        imgs = []
        for s in samples:
            if isinstance(s.imgs, np.ndarray) and s.imgs.size > 0:
                imgs.append(s.imgs)
            elif isinstance(s.imgs, list) and len(s.imgs) > 0:
                imgs.extend(s.imgs)
        ####################################################
        if len(imgs) > 0:
